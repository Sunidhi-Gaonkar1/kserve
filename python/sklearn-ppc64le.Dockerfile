...
RUN apt-get update && apt-get install -y --no-install-recommends python3-dev curl build-essential gcc g++ gfortran libssl-dev pkg-config libopenblas-dev && apt-get clean && \
